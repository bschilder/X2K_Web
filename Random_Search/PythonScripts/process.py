...
# Define grid of options to generate random permutations out of
options_grid = {
    'TF-target gene background database used for enrichment': Pick(1, [
        'ChEA 2015',
        'ENCODE 2015',
    ...
    'kinase interactions to include': Pick(1, [
        'kea 2018',
        'ARCHS4',
        'iPTMnet',
        'NetworkIN',
...
save_file = 'process.csv'


test_kinase_file = 'Kinase_Perturbations_from_GEO_down.txt'
test_kinase = ''
test_input_genes = []
test_options = {}
test_options_binary_array = {}
test_x2k_options = {}
test_x2k_response = ''
test_x2k_results = {}
...
while True:
    try:
        last = time.time()
        kinase, genes = random.choice(kinase_genes)
        options = produce_random_option(options_grid)
        result = options_to_binary_array(options, options_grid)
        ...
        # else:
        #     result['score'] = random.random() * 100
        # result['n_g2n_nodes'] = random.randrange(30, 1000)
        # result['n_sig_kinase'] = random.randrange(0, 20)
        ...
        with open(save_file, 'a') as fh:
            if not save_file_exists:
                print(
                    *convert_result_to_list(result, True),
                    'n_g2n_nodes', 'n_sig_kinase', 'kinase',
                    sep=',', end='\n',
                    file=fh,
                )
                save_file_exists = True
            print(
                *convert_result_to_list(result),
                n_g2n_nodes, n_sig_kinase, kinase,
                sep=',', end='\n',
                file=fh,
            )
        ...
        print(
            ...
            'Last score: %s' % (float(result['score'])),
            'Time since last result: %0.3f' % (time.time() - last),
            'Running Summary:',
            calculate_summary_results(df),
            sep='\n',
        )
        last = time.time()
    except KeyboardInterrupt:
